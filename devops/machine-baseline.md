...
- `machine-setup.md` — Step-by-step how-to guide for humans (the tutorial)
- `machine-baseline.md` — Desired state declaration (this file, the spec)
- `health-check.md` — Ongoing monitoring agent (the controller)
...
Copy to `~/Library/LaunchAgents/` and load via `launchctl load`.
...
**Verify:** `ls ~/.openclaw-config/VERSION && git -C ~/.openclaw-config status`
...
Run these checks to confirm a machine meets baseline. Every check should pass. Any
...
Any line showing `NOT FOUND`, `NOT RUNNING`, `NOT LOADED`, or `MISSING` indicates
baseline drift that needs to be resolved.
